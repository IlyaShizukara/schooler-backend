...
_BASE_SYSTEM_PROMPT = (
    ...
    "заголовков — это чат, а не документ. Формулы и математические "
    "выражения пиши в LaTeX-разметке: одинарные доллары $...$ для формулы "
    "внутри строки, двойные $$...$$ для формулы на отдельной строке — "
    "именно так, без markdown-заголовков/таблиц, но LaTeX-формулы это не "
    "нарушает, чат умеет их рендерить. Не выводи формулу как обычный текст "
    "или псевдографику (x^2, x**2) — используй настоящую LaTeX-нотацию "
    "($x^2$, $\\frac{a}{b}$, $\\sqrt{x}$ и т.п.)."
)

# Опционален — включается/выключается тумблером "Режим: Сократический" на
# фронте (ChatIn.socratic), а не жёстко зашит в промпт, как было раньше.
...
# Служебная разметка шагов решения — фронт парсит её ПОСЛЕ завершения
# стрима (см. ai-chat-panel.tsx/parseStepSolution) и рисует красивые
# карточки шагов вместо парсинга на лету, в духе того же приёма, что и с
# формулами: во время стриминга модель просто пишет обычный текст, а
# структурированный вид строится один раз из уже полного ответа — так
# надёжнее, чем распознавать недописанные маркеры на каждом чанке.
#
# Формат намеренно не JSON — YandexGPT не гарантированно генерирует
# валидный JSON, а обрыв на середине объекта (лимит токенов, сеть) сделал
# бы весь ответ нерендерящимся. Текстовые маркеры [[...]] деградируют
# мягко: если модель не проставила ни одного — просто ничего не находится,
# и фронт показывает ответ как обычный текст (см. parseStepSolution:
# возвращает null, если совпадений нет).
_STRUCTURED_FORMAT_INSTRUCTIONS = (
    "Когда даёшь ПОЛНОЕ пошаговое решение задачи целиком (не при короткой "
    "подсказке, не при наводящем вопросе, не при обычном разговорном "
    "ответе на общий вопрос) — оформляй его так: перед каждым шагом на "
    "отдельной строке пиши маркер вида [[ШАГ:Короткое название шага|ТИП]], "
    "где «Короткое название шага» — 3-6 слов сути этого шага, а ТИП — "
    "ОДНО слово, кратко характеризующее шаг, например: ОБОСНОВАНО (факт "
    "или построение), ТЕОРЕМА (применение теоремы/правила/формулы), "
    "РАСЧЁТ (вычисление), ВЫВОД (логический переход к следующему шагу) — "
    "выбирай подходящее по смыслу слово, необязательно дословно из этого "
    "списка. Сразу после маркера — сам текст шага обычной строкой, с "
    "формулами в $...$ как обычно. После последнего шага, на отдельной "
    "строке, поставь маркер [[РЕЗУЛЬТАТ]] и сразу за ним — итоговый ответ "
    "коротко. Не используй маркеры [[ШАГ:...]] и [[РЕЗУЛЬТАТ]] для "
    "подсказок, наводящих вопросов и обычных разговорных ответов — только "
    "для полного оформленного решения задачи."
)


def _build_system_prompt(profile: UserProfile | None, socratic: bool) -> str:
    """Персонализирует системный промпт под профиль ученика (раньше был
    одинаковым для всех) и собирает его из частей: сократический блок
    теперь включается по тумблеру с фронта, а не зашит намертво."""
    parts = [_BASE_SYSTEM_PROMPT]
    if socratic:
        parts.append(_SOCRATIC_INSTRUCTIONS)
    parts.append(_STRUCTURED_FORMAT_INSTRUCTIONS)

    if profile is not None and (profile.exam_type or profile.grade):
        student_parts = []
        if profile.exam_type:
            student_parts.append(f"готовится к {profile.exam_type}")
        if profile.grade:
            student_parts.append(f"{profile.grade} класс")
        parts.append(
            "Контекст ученика: " + ", ".join(student_parts) + ". Учитывай уровень сложности, "
...
class ChatIn(BaseModel):
    ...
    # Тумблер "Режим: Сократический" на фронте. True по умолчанию — это
    # было поведением до появления самого тумблера, ничего не меняется для
    # существующих клиентов, которые пока не прислали это поле явно.
    socratic: bool = True
...
async def chat(payload: ChatIn, user: User = Depends(get_current_user)) -> StreamingResponse:
    ...
    yandex_messages = (
        [{"role": "system", "text": _build_system_prompt(profile, payload.socratic)}]
